...
foreach ($networkConnections as $network)
{
	...
	if ((time() - rpi_getRuntime()) < (int) $last[0] && (float) $last[1] > (float) ($network['sent'] + 4294967295 * $countSent))
		$countSent++;
	
	if ((time() - rpi_getRuntime()) < (int) $last[0] && (float) $last[2] > (float) ($network['receive'] + 4294967295 * $countReceive))
		$countReceive++;
