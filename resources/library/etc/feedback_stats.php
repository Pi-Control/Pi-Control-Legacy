...
{		
	if (trim(shell_exec('php -f "'.$config['paths']['resources'].'/interface/check.php"')) != 'successful')
		$cron_htaccess_check = 'failed';
}

$cron_match = preg_match('/^\* \* \* \* \* (www-data|root) php -f "'.preg_quote(CRON_PATH, '/').'\/init.php"( > \/dev\/null 2>&1)?( # By Pi Control)?/im', implode("\n", $crontab_lines));
...
$stats['trouble-shooting_pi_cron'] = array('match' => $cron_match, 'paket_status' => trim(exec('dpkg -s php5-cli | grep Status: ')), 'htaccess_check' => $cron_htaccess_check);
