...
function getAllNetworkConnections()
{
	...
	foreach ($streamInterfaces as $streamInterface)
	{
		...
		{
			$output1 = 'Unbekannt';
			$output2 = trim(substr($streamInterface, strpos($streamInterface, 'inet addr:', 0) + 10, strpos($streamInterface, 'P-t-P:', 0) - strpos($streamInterface, 'inet addr:', 0) - 10));
		}
		else
		{
...
function scanAccessPoints($networkConnections, $ssh = NULL)
{
	...
	foreach ($networkConnections as $interface)
	{
		...
		for ($i = 1; $i <= substr_count($streamWlan, 'ESSID:"'); $i += 1)
		{
			...
			{
				if ($wirelessOption['encryptionKey'] == 'off')
					$wirelessOption['encryption'] = 'Offen';
				else
...
function getWeather($type, $postcode, $city)
{
	$location = '';
	...
	elseif ($country_config == 'uk')
	{
		if ($type == 'postcode' && (strlen($postcode) > 8 || strlen($postcode) < 5 || !preg_match('/^([A-Z]{1,2}[0-9][A-Z]?[0-9]?)( )?([0-9]{1}[A-Z]{2})$/', $postcode)))
			return 0;
	}
	...
	if ($json = file_get_contents($yqlQueryUrl))
	{
		...
		$data = array();
		$data['city'] = $obj['location']['city']; // Stadt
		$data['country'] = $obj['location']['country']; // Land
		$data['temp'] = str_replace('.', ',' , round($obj['item']['condition']['temp'])); // Temperatur
		$data['temp_min'] = str_replace('.', ',' , round($obj['item']['forecast']['low'])); // Mindesttemperatur
		$data['temp_max'] = str_replace('.', ',' , round($obj['item']['forecast']['high'])); // Höchsttemperatur
		$data['humidity'] = $obj['atmosphere']['humidity']; // Luftfeuchtigkeit
		$data['wind'] = str_replace('.', ',' , round($obj['wind']['speed'])); // Windstärke
		$data['icon'] = getWeatherIcon($obj['item']['condition']['code']); // Wetter Icon
		$data['description'] = $obj['item']['condition']['text']; // Wetter Beschreibung
