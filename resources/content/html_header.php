...
if (file_exists(PLUGINS_PATH) && is_dir(PLUGINS_PATH))
{
	...
	if (($tpl->getConfig('other.last_plugin_update_check', 0)+86400) < time() || (isset($_GET['s']) && $_GET['s'] == 'plugin_search'))
	{
		...
		if (isset($update_plugins) && !is_array($update_plugins))
			$tpl->setConfig('other.last_plugin_update_check', time());
		else
			$tpl->setConfig('other.last_plugin_update_check', time()-86400);
	}
	
	if (($tpl->getConfig('other.last_update_check', 0)+86400) < time() || (isset($_GET['s'], $_GET['do']) && $_GET['s'] == 'settings' && $_GET['do'] == 'update'))
	{
		$picontrol_update = checkUpdate();
		if (!is_array($picontrol_update))
			$tpl->setConfig('other.last_update_check', time());
		else
			$tpl->setConfig('other.last_update_check', time()-86400);
	}
}
else
	$plugin_available_string = '<strong class="red">Pluginordner nicht gefunden!</strong>';

$tpl->assign('config_slim_header', $tpl->getConfig('other.slim_header', 'true'));
...
$tpl->assign('last_cron_execution', $tpl->getConfig('other.last_cron_execution', 0)+140);
