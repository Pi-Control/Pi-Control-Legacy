...
else
{
	...
	{		
		if (trim(shell_exec('php -f "'.$config['paths']['main'].'/android/appcheck.php echo"')) != 'successful')
			$cron_htaccess_check = 'failed';
	}
	
	$cron_match = preg_match('/^\* \* \* \* \* www\-data php \-f "'.preg_quote(CRON_PATH, '/').'\/init\.php"( )?(# By Pi Control)?/im', implode("\n", $crontab_lines));
	...
	$tpl->assign('cron_paket_status', trim(exec('dpkg -s php5-cli | grep Status: ')));
